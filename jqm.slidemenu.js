...
function slidemenu(sm, only_close) {
	...
	if (!sm.data('slideopen') && !only_close) {
		sm.show().animate({width: '175px', avoidTransforms: false, useTranslate3d: true}, 'fast');
		$(".ui-page-active").css('left', '175px');
		sm.data('slideopen', true);
		if ($(".ui-page-active :jqmData(role='header')").data('position') == 'fixed') {
			$(".ui-page-active :jqmData(slidemenu)").css('margin-left', '180px');
			$("#top-search").hide();
			$("#home-label").text("");
